...
1. `flow_control_requirements_v0.md` — 要件定義（最上位）
2. `flow_control_module_design_v0.md` — モジュール設計（Detection / Forecasting / DetourRouting / Optimization / FeedbackExtractor / RequestHandler の責務、I/O スキーマ、擬似コード）
3. `flow_control_extension_roadmap_v0.md` — 拡張ロードマップ（未解決事項と段階的拡張手順）
4. `flow_control_external_integration_v0.md` — 外部マイクロサービス連携仕様
5. `flow_control_math_approach_v0.md` — 数理アプローチ補助（MILP 定式化、需要予測モデル等の数理詳細）
